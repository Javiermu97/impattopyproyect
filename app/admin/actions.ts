...
export async function createProduct(formData: FormData) {
  ...
  if (!validation.success) throw new Error('Datos de producto inválidos')
...
export async function createCaracteristica(formData: FormData) {
  ...
  if (!validation.success) throw new Error('Datos de característica inválidos')

  const { error } = await supabase.from('caracteristicas').insert(validation.data)
  if (error) throw new Error('No se pudo guardar la característica: ' + error.message)
...
export async function deleteCaracteristica(caracteristicaId: number) {
  ...
  if (error) throw new Error('No se pudo eliminar la característica')
