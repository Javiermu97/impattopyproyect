  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: '700px', margin: '0 auto', fontFamily: 'sans-serif', minHeight: '90vh' }}>
      <div style={{ marginBottom: '20px' }}>
        <Link href={`/admin/products/edit/${caracteristica.producto_id}`} style={{ color: '#A78D5A', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
          ← Volver al producto
        </Link>
      </div>
      ...
      <form action={updateCaracteristica.bind(null, caracteristica.id)} style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.06)', border: '1px solid #eee' }}>
        ...
        {/* ID del Producto (Oculto para mantener la relación) */}
        <input type="hidden" name="producto_id" value={caracteristica.producto_id} />

        <div style={{ marginBottom: '20px' }}>
          <label style={labelStyle}>Título</label>
          <input 
            name="titulo" 
            defaultValue={caracteristica.titulo} 
            required 
            style={inputStyle} 
          />
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <input 
            name="imagen" 
            defaultValue={caracteristica.imagen || ''} 
            style={inputStyle} 
            placeholder="URL de la imagen o icono"
          />
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <textarea 
            name="descripcion" 
            defaultValue={caracteristica.descripcion} 
            required 
            style={{ ...inputStyle, height: '120px', fontFamily: 'inherit' }} 
          />
        </div>
        ...
        <div style={{ marginBottom: '30px', width: '150px' }}>
          ...
          <input 
            name="orden" 
            type="number" 
            defaultValue={caracteristica.orden || 0} 
            style={inputStyle} 
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          <Link href={`/admin/products/edit/${caracteristica.producto_id}`} style={{ 
            flex: 1, 
            padding: '16px', 
            backgroundColor: '#eee', 
            color: '#333', 
