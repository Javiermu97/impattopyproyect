...
// --- INICIO DE DEFINICIONES DE COMPONENTES AUXILIARES ---
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      ...
      setRelatedProducts(allProducts.filter(p => p.id !== currentId));
    }
  }, []);
  ...
  if (!product || !selectedVariant) return <div className="pdp-container"><p>Cargando producto...</p></div>;
        ...
        <div className="pdp-main-layout">
          <div className="pdp-gallery">
            <div className="pdp-main-image-wrapper" onMouseEnter={() => setZoomActive(true)} onMouseLeave={() => setZoomActive(false)} onMouseMove={handleMouseMove}>
              <img src={mainImage} alt={product.name} className="pdp-main-image" style={{ transformOrigin: `${imgPos.x} ${imgPos.y}`, transform: zoomActive ? 'scale(2)' : 'scale(1)' }} />
            </div>
